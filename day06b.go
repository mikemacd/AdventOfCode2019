...
func (t tree) transfers(A, B string) int {
	ancestryA := t.ancestry(A)
	ancestryB := t.ancestry(B)

	l := len(ancestryB)
	if len(ancestryA) < len(ancestryB) {
		l = len(ancestryA)
	}

	for i := 0; i <= l; i++ {
		if ancestryA[i] != ancestryB[i] {
			return len(ancestryA) - i + len(ancestryB) - i - 2
		}
	}
	return 0
}
